...
export default function Pathfinding() {
  ...
  return (
    <section className="py-16 md:py-24 w-full flex flex-col items-center text-center relative px-4 sm:px-0">
      ...
      >
        <div className="inline-block mb-6 px-4 py-1.5 rounded-full bg-[#D48C1C]/10 border border-[#D48C1C]/20 text-[#D48C1C] font-semibold text-sm tracking-wide uppercase">{t("pathfinding", language, "tag")}</div>
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 md:mb-6">
          {t("pathfinding", language, "title1")}<br className="block md:hidden" /><span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E59E25] to-[#D48C1C]"> {t("pathfinding", language, "title2")}</span>
        </h2>
        <p className="text-[#9BA4B5] text-base md:text-lg max-w-2xl mx-auto">{t("pathfinding", language, "subtitle")}</p>
      </motion.div>
      <div className="w-full max-w-4xl flex flex-col items-center relative z-10">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} whileInView={{ opacity: 1, scale: 1, y: 0 }} viewport={{ once: true }}
        ...
        >
          <p className="text-base md:text-lg leading-snug pb-1 text-left">{t("pathfinding", language, "chatUser")}</p>
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <h3 className="text-gray-500 font-medium mb-4">{t("pathfinding", language, "cex")}</h3>
            ...
            <div className="flex w-full justify-between items-center mt-auto pt-4 border-t border-white/[0.05]">
              <div className="flex flex-col gap-0.5 text-red-500/80">
                <span className="text-[10px] uppercase tracking-wider font-semibold">{t("pathfinding", language, "cost")}</span>
              ...
              <div className="flex flex-col gap-0.5 text-gray-600 text-right">
                <span className="text-[10px] uppercase tracking-wider font-semibold text-gray-700">{t("pathfinding", language, "time")}</span>
                <span className="flex items-center justify-end gap-1"><Clock size={16} /> ~2 dias</span>
              </div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
            ...
            <div className="absolute -top-3 bg-gradient-custom text-black text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-[0_4px_24px_rgba(0,0,0,0.2)]">{t("pathfinding", language, "bestRoute")}</div>
            <h3 className="text-white font-bold mb-4 mt-2 text-lg">{t("pathfinding", language, "tts")}</h3>
            <motion.div animate={{ opacity: [0.7, 1, 0.7] }} transition={{ duration: 2, repeat: Infinity }}
              ...
              <span className="text-[#E59E25]">BRL</span> <ArrowRight size={16} className="text-gray-400" /> <span className="text-[#D48C1C]">Rede Subjacente</span> <ArrowRight size={16} className="text-gray-400" /> <span className="text-[#E59E25]">USD</span>
            </motion.div>
            <div className="flex w-full justify-between items-center mt-auto pt-4 border-t border-white/[0.05]">
              <div className="flex items-center gap-1 text-[#E59E25]"><DollarSign size={18} /><span className="font-bold text-xl">0.05%</span></div>
              <div className="flex items-center gap-1 text-[#E59E25]"><Zap size={18} /><span className="font-bold">~3s</span></div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }}
            ...
            <h3 className="text-gray-500 font-medium mb-4 text-center">{t("pathfinding", language, "global")}</h3>
