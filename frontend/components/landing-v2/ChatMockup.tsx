...
export default function ChatMockup({ messages, className = '' }: ChatMockupProps) {
  return (
    <div className={`bg-[#0C1421] rounded-[2rem] border-[6px] border-[#1a1a1a] shadow-[0_8px_32px_rgba(0,0,0,0.3)] overflow-hidden w-full max-w-[320px] mx-auto flex flex-col font-sans ${className}`} style={{ backgroundImage: 'url("https://www.transparenttextures.com/patterns/cubes.png")', opacity: 0.95 }}>
      
      {/* Header */}
      <div className="bg-[#162032] px-4 py-3 flex items-center gap-3 border-b border-white/[0.03] shrink-0 z-10 shadow-sm">
        <div className="w-10 h-10 rounded-full bg-[#0C1421] flex items-center justify-center p-2 border border-white/[0.03]">
        ...
        <div className="flex flex-col">
          ...
          <div className="text-[#00D2FF] text-[11px] mt-1.5 leading-none font-medium">online</div>
        </div>
      </div>
        ...
        {messages.map((msg, idx) => (
          msg.type === 'bot' ? (
            <motion.div 
              ...
              className="self-start bg-[#162032] text-[#e9edef] px-3.5 py-2.5 rounded-2xl rounded-tl-sm text-[14px] shadow-[0_2px_10px_rgba(0,0,0,0.1)] max-w-[88%] leading-relaxed border border-white/[0.03] relative z-10 whitespace-pre-wrap"
            >
              {msg.text}
            ...
            >
              ...
              <div className="flex items-center justify-end gap-1 mt-1.5 opacity-80">
                ...
                <CheckCheck className="w-3.5 h-3.5 text-[#4CA1EF]" />
              </div>
            </motion.div>
          )
        ))}
