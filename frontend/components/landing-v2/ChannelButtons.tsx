...
export default function ChannelButtons({ compact = false, className = "" }: ChannelButtonsProps) {
  ...
  return (
    <div className={`flex flex-col sm:flex-row min-w-0 gap-3 ${className}`.trim()}>
      ...
      <a
        href="/chat"
        className={`inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-[#00D2FF] ${baseSize} font-semibold text-slate-950 transition hover:bg-cyan-300 shadow-[0_0_20px_rgba(34,211,238,0.3)]`}
      >
        <Monitor className={iconSize} />
        <span>Visualização Web</span>
      </a>
    </div>
  );
}
