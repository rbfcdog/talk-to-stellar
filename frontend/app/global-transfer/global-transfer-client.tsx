...
export default function GlobalTransferClient() {
  ...
  const quote = useMemo(() => {
    ...
    const totalFeesUsd = platformFeeUsd + liquidityFeeUsd + offrampFeeUsd + bankFeeUsd + iofUsd + stellarFeeUsd
    const netUsd = Math.max(grossUsd - totalFeesUsd, 0)
    const usdcSettlementAmount = Math.max(grossUsd - platformFeeUsd - liquidityFeeUsd, 0)
    const effectiveCostBps = grossUsd > 0 ? (totalFeesUsd / grossUsd) * 10000 : 0
    const expiresAt = new Date(lastUpdated.getTime() + 7 * 60 * 1000)
    const quoteId = `q_mock_${Math.round(safeAmountBrl * 100)}_${provider}_${payoutRail}`
  ...
  const timeline: TimelineItem[] = useMemo(
    () => [
      ...
      {
        label: "PIX received",
        detail: "PIX partner webhook confirms settled BRL before any value leaves treasury.",
        icon: Banknote,
        status: simulationStarted ? "complete" : "waiting",
      },
      {
        label: "USDC allocated",
        detail: "Liquidity desk or treasury converts the BRL exposure into Stellar USDC.",
        icon: CircleDollarSign,
        status: simulationStarted ? "complete" : "ready",
      },
      {
    ...
    [payoutRail, provider, simulationStarted],
  )
  ...
  const riskFlags = useMemo(() => {
    ...
    return flags
  }, [payoutRail, provider, quote.grossUsd, quote.safeAmountBrl, recipientType])
  ...
  const transferPayload = useMemo(
    () => ({
      ...
      destination: {
        type: payoutRail,
        account_holder: "Recipient Legal Name",
        bank_name: "Wise US Inc or international bank",
        routing_number: payoutRail === "ach" ? "000000000" : undefined,
    ...
    [payoutRail, quote.quoteId, quote.safeAmountBrl, recipientType],
  )
              ...
              <div className="mt-4 overflow-hidden rounded-lg border border-slate-200">
                {[
                  ...
                  <div
                    ...
                    } ${index === 7 ? "font-bold text-emerald-800" : "text-slate-700"}`}
                  >
                    <span>{label}</span>
