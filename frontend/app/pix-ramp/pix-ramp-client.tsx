...
import {
  ETHERFUSE_TESTNET_FEE_BPS,
  ETHERFUSE_TESTNET_FEE_SAMPLE_AMOUNT_BRL,
  TARGET_ASSETS,
  buildAppPath,
  buildRampFeeBridgeEstimate,
  buildRampReceiptFallbackUrl,
  clientEtherfuseOnRampFeeBps,
  clientTtsTransactionFeeBps,
  clientTtsTransactionMinBrl,
  estimatePixOnRampGrossForBrlReceive,
  extractRampReceiptUrl,
  formatApiAmount,
  formatMoney,
  formatPercentValue,
  formatQuoteAmount,
  formatRampAsset,
  friendlyAssetName,
  getPostConversionState,
  getStoredSession,
  normalizeChannelSource,
  normalizeHumanAmount,
  normalizeTargetAsset,
  optionalSettlementAssetCode,
  parseHumanAmount,
  ...
  sleep,
  toPositiveNumber,
  traditionalMethodFeePct,
  userFacingAssetCode,
  type RampMode,
  type RampResponse,
  type TargetAsset,
} from "@/lib/pix-ramp-utils";
...
export default function PixRampClient({
  ...
  function notifyChatAfterPixCompletion(input: {
    ...
    const postConversionState = getPostConversionState(input.completedTransaction, targetAsset);
    const postConversionPending = postConversionState.pending;
    const postConversionSourceAsset = userFacingAssetCode(postConversionState.sourceAsset || targetAsset, targetAsset);
    const postConversionDestinationAsset = userFacingAssetCode(postConversionState.destinationAsset || targetAsset, targetAsset);
    const finalAsset = postConversionPending
      ? postConversionDestinationAsset
      : userFacingAssetCode(
          postConversionState.destinationAsset ||
    ...
    const finalAmount = postConversionPending
      ...
      : finalAsset === "BRL"
        ...
        : String(postConversionState.destinationAmount || input.completedTransaction?.auto_conversion?.destination_amount || finalReceivedAmount || "");
    const intermediateConversionLine = postConversionState.sourceAmount &&
      postConversionState.sourceAsset
      ? L(
          postConversionPending
            ? `Conversão final em andamento: ${formatRampAsset(postConversionState.sourceAmount, postConversionSourceAsset)} para ${friendlyAssetName(postConversionDestinationAsset, language)}`
            : `Recebido primeiro: ${formatRampAsset(postConversionState.sourceAmount, postConversionSourceAsset)}`,
          postConversionPending
            ? `Final conversion in progress: ${formatRampAsset(postConversionState.sourceAmount, postConversionSourceAsset)} to ${friendlyAssetName(postConversionDestinationAsset, language)}`
            : `Received first: ${formatRampAsset(postConversionState.sourceAmount, postConversionSourceAsset)}`
        )
      : "";
    ...
    enqueueWebChatFeedback([
      ...
      postConversionPending
        ? L(`Valor final: ${receivedAmount}`, `Final amount: ${receivedAmount}`)
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {error && (
          <section className="mt-4 rounded-2xl border border-tts-error bg-tts-error/10 p-4 text-sm text-tts-error">
            ...
            {rampMode === "onramp" && !orderId && !operationLocked && /conta pix|pix account/i.test(error) && (
              <button
                className="mt-3 inline-flex rounded-full bg-tts-error px-4 py-2 text-xs font-black uppercase tracking-normal text-tts-deep transition hover:bg-tts-error/90 disabled:opacity-50"
            ...
            {onboardingUrl && (
              <a
                className="mt-3 inline-flex rounded-full bg-tts-error px-4 py-2 text-xs font-black uppercase tracking-normal text-tts-deep"
                href={onboardingUrl}
                target="_blank"
                rel="noreferrer"
              >
        ...
        {config && !config.available && (
          <section className="mt-5 rounded-2xl border border-tts-gold bg-tts-gold-bg p-4 text-sm text-tts-gold">
            ...
            <a
              className="mt-3 inline-flex rounded-full border border-tts-gold px-4 py-2 text-xs font-black uppercase tracking-normal text-tts-gold transition hover:bg-tts-gold-bg"
              href="/mainnet"
            >
        ...
        <section className="tts-op-shell mt-5 hidden gap-3 rounded-xl border border-tts-border bg-tts-surface p-3 shadow-sm backdrop-blur md:grid sm:grid-cols-2">
          ...
          >
            <span className="block text-xs font-black uppercase tracking-normal opacity-70">{L("Adicionar saldo", "Add money")}</span>
          ...
          >
            <span className="block text-xs font-black uppercase tracking-normal opacity-70">{L("Mandar para PIX", "Send to PIX")}</span>
        ...
        {rampMode === "offramp" && (
          <section className="mt-6 grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
            ...
            <div className={`${mobileStage === "receipt" ? "block" : "hidden"} tts-mobile-flow-card tts-op-shell rounded-2xl border border-tts-border bg-tts-surface p-5 text-tts-deep shadow-sm md:block sm:p-6`}>
              ...
              {!temporaryOffRampTestResult ? (
                <div className="mt-8 h-28 rounded-2xl border border-dashed border-tts-border bg-tts-bg/60" />
              ) : (
                <div className="mt-6 space-y-4">
                    ...
                    <div className="rounded-2xl bg-tts-bg/60 p-4">
                      <p className="text-xs font-bold uppercase tracking-normal text-tts-gold">{L("Entrou no seu PIX", "Arrived in your PIX")}</p>
                    ...
                    {temporaryOffRampTestResult.target_brl && (
                      <div className="rounded-2xl bg-tts-bg/60 p-4">
                        <p className="text-xs font-bold uppercase tracking-normal text-tts-gold">{L("Conversão para BRL", "Conversion to BRL")}</p>
                  ...
                  <div className="rounded-2xl border border-tts-confirm bg-tts-confirm/10 p-4 text-tts-confirm">
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
          <div className={`${needsBrowserLoginForPix ? "hidden" : mobileStage === "details" ? "block" : "hidden"} tts-mobile-flow-card tts-op-shell rounded-2xl border border-tts-border bg-tts-surface p-5 text-tts-deep shadow-sm md:block sm:p-6`}>
            ...
            <div className="mt-6 rounded-2xl border border-tts-confirm bg-tts-confirm/10 p-4">
            ...
            {quote && (
              <div className="tts-mobile-soft-hide mt-6 rounded-2xl border border-tts-confirm bg-tts-confirm/10 p-5 md:block">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-normal text-tts-confirm">{L("Pronto", "Ready")}</p>
                ...
                {quoteExpired && (
                  <div className="mt-4 rounded-2xl border border-tts-error bg-tts-error/10 p-4 text-sm font-bold text-tts-error sm:flex sm:items-center sm:justify-between sm:gap-4">
                    ...
                    <button
                      type="button"
                      className="mt-3 inline-flex w-full justify-center rounded-full bg-tts-error px-4 py-2 text-xs font-black uppercase tracking-normal text-tts-surface transition hover:bg-tts-error/90 disabled:opacity-50 sm:mt-0 sm:w-auto"
                ...
                {onboardingUrl && (
                  <div className="mt-4 rounded-2xl border border-tts-gold bg-tts-gold-bg p-4 text-sm text-tts-gold">
                    ...
                    <a className="mt-3 inline-flex rounded-full bg-tts-gold px-4 py-2 text-xs font-black uppercase tracking-normal text-tts-deep" href={onboardingUrl} target="_blank" rel="noreferrer">
                      {L("Abrir cadastro PIX", "Open PIX setup")}
                    </a>
                  </div>
                )}
          ...
          <div className={`${mobileStage === "payment" ? "block" : "hidden"} tts-mobile-flow-card tts-op-shell rounded-2xl border border-tts-border bg-tts-surface p-5 text-tts-deep shadow-sm md:block sm:p-6`}>
            ...
            {!order ? (
              <div className="mt-8 rounded-2xl border border-dashed border-tts-border bg-tts-bg/60 p-8 text-center text-sm text-tts-muted">
                <p>
                  {quoteExpired
              ...
              <>
                    <div className="mt-4 grid gap-3 lg:grid-cols-[220px_1fr] md:mt-6 md:gap-4">
                      <div className="tts-mobile-qr mx-auto w-full max-w-[260px] rounded-2xl p-2 sm:max-w-none sm:p-4" style={{ backgroundColor: "#ffffff", color: "#111827" }}>
                        {qrDataUrl ? (
                          <img
                            src={qrDataUrl}
                            alt="QR Code PIX"
                  ...
                  <div className="grid grid-cols-2 gap-2 md:block md:space-y-3">
                    <div className="tts-mobile-soft-hide rounded-2xl bg-tts-bg/60 p-4 md:block">
                      <p className="text-xs font-bold uppercase tracking-normal text-tts-confirm">{L("Recebedor", "Receiver")}</p>
                      <p className="mt-1 text-lg font-black">TalkToStellar</p>
                    </div>
                    <div className="rounded-2xl bg-tts-bg/60 p-3 md:p-4">
                      <p className="text-xs font-bold uppercase tracking-normal text-tts-confirm">{L("Valor", "Amount")}</p>
                      <p className="mt-1 text-base font-black md:text-lg">{effectiveOnRampPixPayDisplay}</p>
                    </div>
                    <div className="rounded-2xl bg-tts-bg/60 p-3 md:p-4">
                      <p className="text-xs font-bold uppercase tracking-normal text-tts-confirm">{L("Expira em", "Expires in")}</p>
                      <p className="mt-1 text-base font-black md:text-lg">{quoteCountdown}</p>
                    </div>
                  </div>
                </div>

                  {isSandboxMockOrder && !localMockFallbackAllowed ? (
                    <div className="mt-5 rounded-2xl border border-tts-error bg-tts-error/10 p-4 text-sm font-bold text-tts-error">
                      <p className="text-xs font-black uppercase tracking-normal text-tts-error">{L("PIX expirado", "Expired PIX")}</p>
                      <p className="mt-2">
                    ...
                    <div className="mt-5 rounded-2xl bg-tts-bg/60 p-4">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-xs font-bold uppercase tracking-normal text-tts-confirm">{L("Código PIX copia e cola", "Copy-and-paste PIX code")}</p>
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-2xl border border-tts-border bg-tts-surface p-5 shadow-sm shadow-black/30 sm:p-6">
            <p className="text-xs font-black uppercase tracking-normal text-tts-error">Teste interno de retirada</p>
            ...
            <div className="relative p-5 sm:p-7">
              ...
              {transferFlow && (
                <div className={`relative mt-5 rounded-2xl border p-5 ${pixFundedTransferResult?.transaction_hash ? "border-tts-gold bg-tts-gold-bg" : "border-tts-gold bg-tts-gold-bg"}`}>
                  <p className="text-xs font-black uppercase tracking-normal text-tts-gold">{L("Transferência após PIX", "Transfer after PIX")}</p>
                  {pixFundedTransferResult?.transaction_hash ? (
                    <>
...
function TemporaryEndpointCard({ title, endpoint, description, disabled, hidden, onRun, result }: {
  ...
  return (
    <div className="rounded-2xl border border-tts-border bg-tts-surface p-5 shadow-sm shadow-black/30 sm:p-6">
      <p className="text-xs font-black uppercase tracking-normal text-tts-gold">Endpoint in frontend</p>
  ...
  return (
    <div className="mt-5 rounded-2xl border border-tts-border bg-tts-bg/60 p-3">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-bold text-tts-deep">
        ...
        <div className="rounded-2xl border border-tts-gold bg-tts-gold-bg p-3 text-tts-deep">
          <span className="block uppercase tracking-normal text-tts-gold">{L("App", "App")}</span>
  ...
  return (
    <div className="rounded-2xl bg-tts-surface p-5 text-tts-deep shadow-sm shadow-emerald-950/20 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
	          <p className="text-xs font-black uppercase tracking-normal text-tts-confirm">Debug interno</p>
        ...
        <button className="w-fit rounded-full border border-tts-border px-4 py-2 text-xs font-bold uppercase tracking-normal text-tts-muted transition hover:text-tts-deep disabled:opacity-40" disabled={logs.length === 0} onClick={onClear}>
          Clear logs
        </button>
      </div>
      <div className="mt-5 grid gap-3">
        {logs.length === 0 ? (
          ...
          <details key={log.id} className="rounded-2xl border border-tts-border bg-tts-bg/60 p-4" open={Boolean(log.error)}>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              <div>
                <p className="mb-2 text-xs font-black uppercase tracking-normal text-tts-confirm">Request</p>
              ...
              <div>
                <p className="mb-2 text-xs font-black uppercase tracking-normal text-tts-confirm">Response</p>
...
function AssetMovement({ title, before, after, deltas }: {
  ...
  return (
    <div className="rounded-2xl border border-tts-border bg-tts-surface p-5 text-tts-deep shadow-sm sm:p-6">
      <p className="text-xs font-black uppercase tracking-normal text-tts-gold">Account balances changing</p>
...
function ReceiptRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-tts-bg/60 p-4">
      <dt className="text-xs font-bold uppercase tracking-normal text-tts-confirm">{label}</dt>
