...
describe("UX copy guardrails", () => {
  ...
  it("keeps application options tied to configured vaults and gives recovery actions for PIX shortage", () => {
    ...
    expect(reviewText).toContain("InvestmentGraphs");
    ...
    expect(reviewText).not.toContain("Linhas da carteira");
    expect(reviewText).not.toContain("Portfolio lines");
    expect(reviewText).not.toContain("Real position by asset. Each card also shows the growth path.");
