...
Frontend documentation inventory and freshness guidance: [`DOCS-SUMMARY.md`](./DOCS-SUMMARY.md).

## Folder map

```text
app/            Next.js app router pages, layouts, and route handlers
app/api/        Frontend API proxy routes to backend services
components/     Reusable UI and product components
hooks/          React hooks
lib/            Client/server utilities and domain helpers
public/         Static assets
__tests__/      Vitest and Playwright tests
```

Global styles live in `app/globals.css`. Do not add a second global styles folder; `components.json` and `app/layout.tsx` both point at `app/globals.css`.
