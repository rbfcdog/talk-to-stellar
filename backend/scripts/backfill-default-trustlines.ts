...
async function main(): Promise<void> {
  ...
  const sessionIds = Array.from(new Set((wallets || []).map((wallet: any) => String(wallet.session_id || '').trim()).filter(Boolean)));
  ...
  for (const wallet of wallets || []) {
    ...
    try {
      ...
      console.log(`[${processed}] ${publicKey} -> assets_created=${result.assets.length} errors=${result.errors.length}`);
    } catch (error) {
      failed += 1;
