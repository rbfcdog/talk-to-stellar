...
const INTENT_ROUTING_SPECS: Array<{ intent: IntentType; toolName: string; description: string }> = [
  {
    ...
    description: 'Use for own-account PIX entrada/on-ramp only: the user wants to put/add/load/deposit/bring/receive money into their own TalkToStellar account via PIX. Portuguese examples: "colocar 100 reais via pix", "me ajude com o colocar 100 reais via pix", "me ajuda a adicionar 100 reais por PIX", "adicionar saldo com pix de 100 reais", "depositar via PIX", "trazer 50 reais via PIX", "receber um PIX na minha conta", "uero mandar um pix pra chegar 100 usdc na minha conta". CRITICAL: only use this when PIX is the rail. DO NOT use when the user mentions "bridge", "wire", "ACH", "(USD)", "dolar/dollars", "banco americano", or "US bank" — bringing money in "via bridge" / "via wire" / "from my US bank" is a USD wire/ACH deposit (WIRE_ONRAMP / open_wire_onramp_interface), NOT PIX. "trazer dinheiro pra minha conta via bridge", "bring money to my account via bridge", "i want to bring money via bridge", "trazer dolares via bridge" are NOT this route — they are WIRE_ONRAMP. This route never requires a contact, email, phone, recipient, destination public key, saved contact, or human recipient. Words like chegar, entrar, cair, receber plus "na minha conta" describe the target balance in the user own account, not a contact. For own-account PIX on-ramp, asset_code is the exact first asset/amount the PIX funding should deliver to the user account. Reais/BRL -> asset_code=BRL. "chegar 100 USDC na minha conta" -> amount=100, asset_code=USDC. If the user gives two assets for their own account, preserve the sequence: "colocar 100 XLM pra receber em USDC" means fund exact 100 XLM first, then convert that XLM to USDC, so amount=100, asset_code=XLM, dest_asset_code=USDC, recipient_query empty. Do not rewrite that as 100 USDC or as BRL. The PIX payment itself is collected in BRL and the page calculates the BRL amount dynamically. Never use this if the message has PIX plus a named recipient/person after pra/para/pro/a, such as "fazer PIX pra Ana Silva de 100 XLM" or typo "uero fazer pix pra ana silva de 100 xlm". Those are route_pix_intent because PIX funds a payment to that contact. In those cases the amount/asset is the final recipient target, not BRL for the sender own account.',
  },
  {
    intent: IntentType.PIX,
    toolName: 'route_pix_offramp_intent',
    description: 'Use for PIX saída/off-ramp only: the user wants money to leave their TalkToStellar account to their own PIX/bank/key. Portuguese examples: "sacar 50 reais para meu PIX", "retirar 20 USDC para minha chave PIX", "mandar pra fora 50 reais em pix", "uero mandar 100 reais pra fora do pix", "tirar da conta para o banco", "quero mandar 10 usdc pra fora da conta", "mandar 10 usdc pra fora da minha conta", "mandar 10 usdc para fora da conta". DO NOT use when the user mentions "bridge", "(USD)", "dólar/dollars", "wire", "ACH", or "banco americano/US bank" — that is a USD withdrawal to a US bank (WIRE_ONRAMP family / open_usd_withdraw_interface), NOT PIX, even with "pra fora". "enviar dinheiro pra fora via bridge" and "enviar pra fora (USD) via bridge" are NOT this route. This route wins over normal payment for own-account money-out wording even when PIX is omitted. "pra fora da conta", "para fora da minha conta", "tirar da conta", and "sacar da conta" are own-account exit requests, not missing contact-payment recipients; leave recipient_query empty unless the user gives their own PIX key. Do not use this for plain external-transfer wording with a conversion layer like "uero mandar 10 usdc em xlm pra fora"; that is route_payment_intent with source_asset_code=USDC, dest_asset_code=XLM, amount=10, and needs_clarification=true if the external destination/key is missing.',
  },
  {
    intent: IntentType.PIX,
  ...
  },
  {
    ...
    description: 'Use when the user asks for account balance/saldo, available money, quanto tenho, sald9, sald0, saldp, balances, or current holdings in the wallet. Account values are sensitive: route to the secure balance screen and never show balances or amounts in chat.',
  },
  {
  ...
  },
  {
    ...
    description: 'Use when the user wants to bring, receive, add, or deposit USD/dollars from a US bank account via wire transfer or ACH, OR simply wants to SEE/OPEN their dollar account (the "Bridge" account) and its deposit details. Portuguese examples: "quero trazer dólares do meu banco americano", "depositar via wire", "trazer money dos EUA", "receber USD de fora", "wire 500 dolares", "trazer 500 USD via banco nos EUA", "quero depositar em dólar via transferência bancária". Also use for any "bring money in via bridge / wire / ACH" request EVEN when the verb looks like a PIX deposit verb (trazer/colocar/adicionar/depositar/receber): "trazer dinheiro pra minha conta via bridge", "bring money to my account via bridge", "i want to bring money via bridge", "colocar dinheiro via bridge", "trazer dolares via bridge", "receber via bridge", "depositar via wire". The word "bridge"/"wire"/"ACH" overrides PIX — these are USD deposits, never PIX. Also use for view/open-account requests like: "quero ver a bridge", "want to see bridge", "show me the bridge", "ver minha conta de dólar", "see my dollar account", "abrir conta de dólar", "minhas contas de dólar", "open my dollar account". This is distinct from PIX (which is BRL via Brazilian banking). Never use for PIX on-ramp, PIX off-ramp, or BRL/USDC swap flows. Use for any "dollar account / bridge" view request or when the source is a foreign/US bank account and the user wants USD to arrive in their TalkToStellar balance.',
  },
  {
  ...
  },
  {
    ...
    description: 'Use only for greetings, broad help/menu/capability questions, educational explanations, unsupported small talk, or messages that are truly not an actionable TalkToStellar request. When the user asks to explain assets, PIX, earnings, conversions, payments, security, or account concepts, set explanation_topic to the matching topic. Never use for actionable product requests, PIN/security changes, or money-transfer requests that contain a transfer verb, amount, asset, and recipient. NEVER use for requests that mention "bridge", "conta de dólar", "dollar account", "wire", or "ACH" — "ver a bridge" / "i want to see bridge" / "trazer via bridge" / "enviar pra fora via bridge" are actionable WIRE_ONRAMP/USD requests, not capability questions; do not answer them with the generic account menu.',
  },
];
    ...
    parameters: {
      ...
      properties: {
        ...
        quote_asset_code: {
          type: 'string',
          enum: ['BRL', 'USDC', 'CETES', 'XLM', ''],
          description: 'Optional quote/reference asset for informational quote-only wording. Do not use this to downgrade an exact own-account receive request to BRL. Example: "chegar 100 USDC na minha conta" must use asset_code=USDC and leave quote_asset_code empty unless the user explicitly says USDC is only a reference.',
        },
        source_asset_code: {
          type: 'string',
          enum: ['BRL', 'USDC', 'CETES', 'XLM', ''],
...
export class AgentGraph {
  ...
  private pixFundedPaymentIntentFromLlmRoute(state: AgentState): {
    ...
    const destinationAssetCode = this.normalizeAgentAssetCode(
      route.dest_asset_code ||
        route.destination_asset_code ||
        route.receive_asset_code ||
        route.receiveAssetCode ||
    ...
    return {
      ...
      amount_currency: assetCode || 'BRL',
      asset_code: assetCode || 'BRL',
      pay_destination_asset_code: destinationAssetCode && destinationAssetCode !== (assetCode || 'BRL')
  ...
  private pixOwnAccountIntentFromLlmRoute(state: AgentState): {
    ...
    return {
      ...
      post_conversion_asset_code: direction === 'onramp' && destAssetCode && destAssetCode !== normalizedAsset
        ? destAssetCode
        : undefined,
      quote_asset_code: quoteAssetCode || undefined,
      quote_amount: quoteAmount || (quoteAssetCode ? amount : undefined),
    };
  }
  ...
  private async buildPixRampUrl(state: AgentState, intent: {
    ...
    const normalizedAssetCode = this.normalizeAgentAssetCode(intent.asset_code || '');
    const normalizedAmountCurrency = this.normalizeAgentAssetCode(intent.amount_currency || normalizedAssetCode || '');
    const quoteAssetCode = this.normalizeAgentAssetCode(intent.quote_asset_code || '');
    const postConversionAssetCode = intent.direction === 'onramp' && intent.flow !== 'fund_and_pay'
      ? this.normalizeAgentAssetCode(intent.post_conversion_asset_code || '')
      : '';
    const onRampFinalAsset = intent.direction === 'onramp' && intent.flow !== 'fund_and_pay'
      ? (quoteAssetCode && quoteAssetCode !== 'BRL' ? quoteAssetCode : (normalizedAssetCode || normalizedAmountCurrency || 'BRL'))
      : (normalizedAssetCode || normalizedAmountCurrency || 'BRL');
    const amountCurrency = normalizedAmountCurrency || onRampFinalAsset;
    const isExactOnRampReceive = Boolean(
      intent.direction === 'onramp' &&
      intent.flow !== 'fund_and_pay' &&
      onRampFinalAsset !== 'BRL' &&
      (amountCurrency === onRampFinalAsset || (quoteAssetCode && quoteAssetCode !== 'BRL'))
    );
    const urlAsset = intent.direction === 'onramp'
      ? (isExactOnRampReceive ? onRampFinalAsset : (amountCurrency === 'BRL' ? 'BRL' : onRampFinalAsset))
      : normalizedAssetCode || amountCurrency;
    url.searchParams.set('mode', intent.direction);
    url.searchParams.set('asset', urlAsset);
    if (intent.direction === 'onramp' && onRampFinalAsset !== 'BRL') {
      url.searchParams.set('target_asset', onRampFinalAsset);
    }
    if (postConversionAssetCode && postConversionAssetCode !== onRampFinalAsset) {
      url.searchParams.set('flow', 'fund_and_convert');
      url.searchParams.set('post_conversion_asset', postConversionAssetCode);
      url.searchParams.set('convert_to_asset', postConversionAssetCode);
    }
    ...
    if (intent.amount) {
      if (isPixFundedPayment && finalPayAmount && finalPayAsset) {
        ...
        url.searchParams.set('receive_amount', intent.quote_amount || intent.amount);
        url.searchParams.set('receive_asset', onRampFinalAsset);
      } else if (intent.direction === 'onramp' && urlAsset === 'BRL') {
        url.searchParams.set('amount', intent.amount);
        url.searchParams.set('currency', 'BRL');
        if (onRampFinalAsset === 'BRL') {
          url.searchParams.set('receive_amount', intent.amount);
          url.searchParams.set('receive_asset', 'BRL');
        }
      } else if (intent.direction === 'onramp') {
        url.searchParams.set('amount', intent.amount);
  ...
  private async handlePixRampRequest(state: AgentState): Promise<AgentState> {
    ...
    if (!intent.is_pix_ramp) {
      ...
      if (intent.direction === 'offramp') {
        ...
        const amountText = this.formatMoneyByAsset(intent.amount, intent.amount_currency || 'BRL');
        const deliveryAssetCode = this.normalizeAgentAssetCode((intent as any).pay_destination_asset_code || '');
        ...
        const deliverySuffix = deliveryAssetCode && deliveryAssetCode !== sourceAssetCode
          ? this.text(
              language,
              ` para entregar em ${deliveryAssetCode}`,
              ` to deliver in ${deliveryAssetCode}`
            )
          : '';
        state.response_message = this.text(
          language,
          `Para mandar ${amountText} para ${resolvedRecipientLabel}${deliverySuffix} via PIX, abra:\n\n${url}\n\n${pixFeeNote}\n\nA tela mostra o valor final, pede seu PIN e envia para ${resolvedRecipientLabel} automaticamente depois da confirmação.`,
          `To send ${amountText} to ${resolvedRecipientLabel}${deliverySuffix} with PIX, open:\n\n${url}\n\n${pixFeeNote}\n\nThe page shows the final amount, asks for your PIN, and sends it to ${resolvedRecipientLabel} automatically after confirmation.`
        );
      } else {
        const quoteAssetCode = this.normalizeAgentAssetCode((intent as any).quote_asset_code || '');
        const postConversionAssetCode = this.normalizeAgentAssetCode((intent as any).post_conversion_asset_code || '');
        const finalAssetCode = quoteAssetCode && quoteAssetCode !== 'BRL'
          ? quoteAssetCode
          : this.normalizeAgentAssetCode(intent.asset_code || intent.amount_currency || 'BRL');
        const amountText = this.formatMoneyByAsset(intent.amount, finalAssetCode || 'BRL');
        const actionText = postConversionAssetCode && postConversionAssetCode !== finalAssetCode
          ? this.text(
              language,
              `receber ${amountText} na sua conta e depois converter para ${this.formatUserFacingAssetName(postConversionAssetCode, language)}`,
              `receive ${amountText} in your account and then convert it to ${this.formatUserFacingAssetName(postConversionAssetCode, language)}`
            )
          : this.text(language, `receber ${amountText} na sua conta`, `receive ${amountText} in your account`);
        const deliveryAssetName = postConversionAssetCode && postConversionAssetCode !== finalAssetCode
          ? `${finalAssetCode} e converter para ${postConversionAssetCode}`
          : finalAssetCode === 'USDC'
            ? 'USDC'
            : this.formatUserFacingAssetName(finalAssetCode || 'BRL', language);
        state.response_message = this.text(
          language,
          `Tudo finalizado. Aqui estão suas informações para ${actionText} via PIX:\n\n${url}\n\n${pixFeeNote}\n\nNa página, o PIX a pagar já inclui a taxa por fora para o saldo entrar como ${deliveryAssetName}.`,
          `Done. Here are your details to ${actionText} with PIX:\n\n${url}\n\n${pixFeeNote}\n\nOn the page, the PIX amount to pay already includes the fee on top so the balance arrives as ${deliveryAssetName}.`
        );
      }
    }
  ...
  private async buildRuntimeContext(userId?: string, sessionId?: string, language: 'pt-BR' | 'en' = 'pt-BR'): Promise<string> {
    ...
    return [
      ...
      '- If the user asks for an explanation, choose exactly one explanation topic: pix, assets, earnings, conversion, payments, security, or account. Never request all explanations in one answer. If the request is broad, pick the most relevant topic from the current message and keep it short.',
      ...
      '- Do not send duplicate welcome/start messages. On first login/onboarding completion, prefer a practical first-steps tutorial: bring money with PIX, check balance, then convert/send/invest, then view history.',
      '- For first greetings tied to first login/onboarding, guide the user step by step to bring money with PIX, check balance, do one first action, and view history. If the user explicitly asks for ajuda, funcionalidades, comandos, or what TalkToStellar can do, use get_intent_help and show the fuller capability list.',
    ...
    return [
      new SystemMessage({
        content: [
          ...
          'Exact audit example: "uero mandar um pix pra chegar 100 usdc na minha conta" must be route_pix_onramp_intent with amount=100, asset_code=USDC. "chegar" is not a recipient; "na minha conta" means own-account on-ramp. The PIX is paid in BRL, but the final balance delivered to the account is USDC.',
          'Exact audit example: "uero colocar 100 xlm pra eu receber em usdc" must be route_pix_onramp_intent with amount=100, asset_code=XLM, dest_asset_code=USDC, recipient_query empty. This means fund 100 XLM first via PIX, then convert that 100 XLM to USDC in the same account.',
          'If the latest user says "minha conta", "pra mim", "eu receber", or corrects "nso, eh pra minha conta mesmo", clear any stale recipient from previous context.',
  ...
  private routeCandidateFromToolCall(call: { name: string; args?: Record<string, any> }): IntentRouteCandidate | null {
    ...
    const quoteAssetCode = this.normalizeAgentAssetCode(call.args?.quote_asset_code || call.args?.quoteAssetCode || '');
    const rawQuoteAmount = String(call.args?.quote_amount || call.args?.quoteAmount || '').trim();
    const quoteAmount = rawQuoteAmount ? normalizeHumanAmountText(rawQuoteAmount) : undefined;
    ...
    return {
      ...
      quoteAssetCode: quoteAssetCode || undefined,
  ...
  private serializeIntentRouteCandidate(candidate: IntentRouteCandidate | null): Record<string, any> | undefined {
    ...
    return {
      ...
      quote_asset_code: candidate.quoteAssetCode,
...
- For route_general_intent educational explanations, fill exactly one explanation_topic. Use assets for "quais são os assets/explique cada ativo", pix for PIX explanations, earnings for rendimentos/vault explanations, conversion for conversion explanations, payments for payment/link explanations, security for PIN/passkey/security, and account for account/profile/login concepts. Do not use all when a single topic can be inferred. For broad help/capability/menu requests, route to get_intent_help instead of explanation_topic=all.
...
- For route_pix_intent, amount and asset_code are the final amount and asset the recipient should receive unless the user explicitly names a separate delivery asset. Example: "quero fazer pix pra Ana Silva de 100 XLM" -> route_pix_intent with amount="100", asset_code="XLM", recipient_query="Ana Silva". Example: "want to send 100 USDC PIX to Ana Silva so they receive in CETES" -> route_pix_intent with amount="100", asset_code="USDC", dest_asset_code="CETES", recipient_query="Ana Silva".
...
- route_balance_intent: user asks to see balance, saldo, holdings, available money, quanto tenho, current wallet amount, or any asset balance such as XLM/USDC/CETES/BRL. Balance values are sensitive; never answer with amounts in chat. The backend opens the secure balance screen with PIN.
...
- "mandar um PIX pra chegar 100 USDC na minha conta" and "fazer PIX para cair 100 dólares na minha conta" are own-account on-ramp with asset_code=USDC because the requested final account balance is USDC. "PIX para entrar 50 XLM na minha conta" is an own-account on-ramp with asset_code=XLM. Do not treat "chegar/entrar/cair" as recipient_query.
- "colocar 100 XLM pra eu receber em USDC", "botar 100 XLM e receber em dólar na minha conta", and typo variants like "uero colocar 100 xlm pra eu receber em usdc" are own-account sequential on-ramp: route_pix_onramp_intent, amount=100, asset_code=XLM, dest_asset_code=USDC, recipient_query empty. The sequence is PIX -> receive 100 XLM -> convert 100 XLM to USDC. Do not change the amount to 100 USDC. Do not carry Ana Silva or any previous recipient into this route.
- If a follow-up corrects the destination to "minha conta mesmo", own-account wording overrides stale recipient context. Use route_pix_onramp_intent or route_conversion_intent according to the money flow, and leave recipient_query empty.
...
- "fazer PIX pra Ana Silva de 100 XLM", "uero fazer pix pra ana silva de 100 xlm", "mandar PIX para Carlos de 20 USDC", and "pagar Ana via PIX" are route_pix_intent because PIX is funding a payment to a contact. They are not own-account on-ramp. The asset after the amount is the final asset for the recipient. "want to send 100 usdc pix to ana silva so they receive in CETES" is route_pix_intent with asset_code=USDC and dest_asset_code=CETES.
...
- risk=high for money movement, PIN/security, login/logout, account access, profile/wallet access, or any action affecting funds.`;
  ...
  private formatOwnReceivingKeysForLanguage(language: 'pt-BR' | 'en', publicKey?: string, pixKey?: string, preferPublicKey = false): string {
    if (preferPublicKey) {
      if (!publicKey) {
        return this.text(
          language,
          'Não encontrei uma chave pública Stellar vinculada à sua sessão atual.',
          'I could not find a Stellar public key linked to your current session.'
        );
      }
      return this.text(
        language,
        `Sua chave pública Stellar é:\n\`${publicKey}\``,
        `Your Stellar public key is:\n\`${publicKey}\``
      );
    }
