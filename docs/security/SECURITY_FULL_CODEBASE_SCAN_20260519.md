...
Runtime startup migrations and the legacy `exec_sql` runner were removed. `backend/migrations/20260613_00_full_schema.sql` is the only schema source and must be applied from a trusted database-admin context.
...
Issue:
The consolidated bootstrap finishes by dropping `public.exec_sql`, revoking Vault helper execution from public roles, enabling and forcing RLS on the protected table set, and revoking table access from `PUBLIC`, `anon` and `authenticated`. Existing deployed databases receive that posture only after the bootstrap is applied from a trusted Supabase admin context.

Why it matters:
Runtime startup migrations and legacy setup files were removed, but an existing database can still retain an older policy state until the consolidated bootstrap is applied and verified.

Recommended fix:
- Apply `backend/migrations/20260613_00_full_schema.sql` in Supabase SQL Editor or through a trusted admin connection.
- Keep runtime migration execution removed.
...
The consolidated migration finishes by:
...
- Do not restore runtime startup migrations.
