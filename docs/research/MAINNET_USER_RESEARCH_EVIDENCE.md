...
Em producao, aplique `backend/migrations/20260613_00_full_schema.sql` pelo fluxo seguro do Supabase/CI. O runner `migrate:required` usa `psql` e exige `DATABASE_URL` ou `SUPABASE_DB_URL`.
