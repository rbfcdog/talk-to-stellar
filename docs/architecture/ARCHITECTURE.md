...
TalkToStellar is a real-time value transfer platform via Stellar, integrated with messaging applications (WhatsApp, Telegram). Users talk to AI agents that interpret commands and execute financial transactions securely.
...
- **Páginas principais**:
  - `/onboarding`: User registration (Passkey + Password)
  - `/confirm-payment?token=<jwt>`: Secure payment confirmation
...
- **Mode**: Polling (bot constantly queries Telegram)
- **Fluxo**:
  1. User writes on Telegram
  2. Bot extrai `provider_user_id` (Telegram user ID)
  3. Bot chama `/api/external/check-account` para validar
  4. If it doesn't exist, send an onboarding link via `creationUrl`
  5. If it exists, forward the message to `/api/agent/query`
- **Session Store**: In-memory (bot localStorage)

#### **WhatsApp/Twilio Webhook**
- **Localização**: `twilio-webhook/`
- **Tecnologia**: Node.js, Twilio
- **Mode**: Webhook (Twilio sends POST when receiving message)
...
4. LLM returns natural language response
...
- Stores Stellar private keys in a secure manner
...
### 3.2 Payment Flow (XLM Payment Path → USDC)
...
### 3.3 Telegram Account Verification Flow
...
- **Created by**: Backend when initiating payment
