    ...
    Practical result:
    - User cannot “Confirm and receive” directly without account readiness.
    ...
    backend/src/db/legacy-agent-bootstrap.ts
    ...
    `backend/src/db/legacy-agent-bootstrap.ts` was updated with new tables, but there are also standalone SQL migrations. Keep both in sync if startup migrations remain enabled.
