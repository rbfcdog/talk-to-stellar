...
```bash
mkdir -p docs/insta-awards/docs/deliverable-3/evidence/json

curl -s "${BACKEND_URL}/api/transfers/${TRANSFER_ID}/workflow" \
  -H "Authorization: Bearer ${OPS_TOKEN}" \
  | jq > docs/insta-awards/docs/deliverable-3/evidence/json/workflow.json

curl -s "${BACKEND_URL}/api/transfers/${TRANSFER_ID}/reviewer-evidence" \
  -H "Authorization: Bearer ${OPS_TOKEN}" \
  | jq > docs/insta-awards/docs/deliverable-3/evidence/json/reviewer-evidence.json

curl -s "${BACKEND_URL}/api/transfers/${TRANSFER_ID}/payout-evidence" \
  -H "Authorization: Bearer ${OPS_TOKEN}" \
  | jq > docs/insta-awards/docs/deliverable-3/evidence/json/payout-evidence.json

curl -s "${BACKEND_URL}/api/transfers/${TRANSFER_ID}/orchestration-log" \
  -H "Authorization: Bearer ${OPS_TOKEN}" \
  | jq > docs/insta-awards/docs/deliverable-3/evidence/json/orchestration-log.json

curl -s "${BACKEND_URL}/api/transfers/${TRANSFER_ID}/reconciliation" \
  -H "Authorization: Bearer ${OPS_TOKEN}" \
  | jq > docs/insta-awards/docs/deliverable-3/evidence/json/reconciliation.json
```
...
```bash
mkdir -p docs/insta-awards/docs/deliverable-3/evidence/screenshots
...
```text
docs/insta-awards/docs/deliverable-3/runs/<timestamp>.md
```
