...
The current implementation is an orchestration layer. It does not claim regulated money transmission, does not execute live ACH/wire by default, and keeps payout execution mocked unless explicitly configured.
...
- Stellar settlement evidence step at `POST /api/transfers/:id/settle-stellar`.
- USD payout instruction adapter at `POST /api/transfers/:id/payout-instruction`.
- Reconciliation report at `GET /api/transfers/:id/reconciliation`.
- Mock, Circle compatibility, and Bridge compatibility payout adapters.
...
If those Stellar values are missing, the settlement step still works but produces mock settlement evidence.
...
ETHERFUSE_WEBHOOK_SECRET=the-same-secret-used-in-backend
```
...
The frontend proxy injects the Etherfuse webhook secret when the tester simulates a Pix webhook.
...
4. `Pix paid`
   - Calls `POST /api/webhooks/etherfuse/pix`.
   - Simulates Etherfuse confirming Pix settlement.
...
5. `Stellar`
   ...
   - If Stellar secrets are not configured, this creates mock evidence.
...
`Run sandbox flow` executes the same steps in sequence using mock Pix and mock payout.
...
- Set the same `ETHERFUSE_WEBHOOK_SECRET` in backend and frontend.
- Map the real Etherfuse transaction/order ID to `pix_order_id` and `pix_payment_id`.
- Add webhook replay/idempotency checks if Etherfuse sends event IDs.

Current status:

- The service already wraps existing Etherfuse customer, quote, and on-ramp calls.
- The demo can also use explicit mock Pix intent for reviewer walkthroughs.
...
- If not configured, it creates mock settlement evidence.
...
- `PAYOUT_PROVIDER=mock` is demo-ready.
- `PAYOUT_PROVIDER=circle` and `PAYOUT_PROVIDER=bridge` prepare provider-shaped payloads.
...
- Screenshot after `Pix paid`.
- Screenshot after `Stellar`.
- Screenshot after `Payout`.
...
4. Open `/international-transfer` and run `Run sandbox flow`.
...
8. Replace mock provider with a provider compatibility adapter in sandbox.
