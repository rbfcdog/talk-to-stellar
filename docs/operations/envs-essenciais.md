...
O guia completo fica em `docs/operations/env-geral-para-todos.md`.
...
# Amostra usada para derivar referencia USDC <-> TESOURO/Real pela propria rota.
...
`USD_BRL_SANITY_MIN` e `USD_BRL_SANITY_MAX` bloqueiam rotas testnet ruins. Ex.: se a rota devolver `1 USDC = R$ 1,19`, o backend rejeita o link de confirmacao em vez de mostrar uma conversao enganosa. O sistema nao compara com mercado externo.
...
Se o banco for novo, aplique tambem as migrations base listadas em `docs/operations/session-env-and-migrations.md`.
