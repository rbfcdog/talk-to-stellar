...
**Circle payout foundation**: after `USDC_SETTLED`, `backend/src/api/services/international-transfer.service.ts` calls the selected payout adapter with USDC rail metadata. `backend/src/api/services/usd-payout-adapters.ts` builds Circle Mint `/v1/businessAccount/payouts` instructions, sends `settlement_asset_code=USDC`, `off_ramp_source_asset_code=USDC`, `route=PIX_BRL_TO_STELLAR_USDC_TO_USD_BANK`, and the Stellar settlement hash in metadata, polls payout status, normalizes Circle webhook events, and persists redacted provider evidence. Sandbox/live execution still requires `ENABLE_REAL_PAYOUT_EXECUTION=true`, `CIRCLE_API_KEY`, and a linked Circle bank account ID.
