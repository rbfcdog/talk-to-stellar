...
|------|-------|-------|---------|---------------|
| [`DATA-MODEL.md`](./DATA-MODEL.md) | DATA-MODEL.md — Database Schema | 953 | Supabase PostgreSQL. No ORM — raw queries via `@supabase/supabase-js`. Repository pattern. The complete SQL schema lives only in `backend/migrations/20260613_00_full_schema.sql`. It replaces the former incremental migrations and runtime bootstrap code. | English or mostly English. |
