...
| [`ops-dashboard.md`](./ops-dashboard.md) | Ops Dashboard - Surface Audit | 663 | Backend `/ops` now uses centralized dashboard tokens, reusable server-rendered primitives, metrics, filters, pagination, in-place refresh, responsive rows, and transfer forensic detail rendering. Live list screenshots were captured; detail screenshots await a seeded lifecycle transfer. | English or mostly English. |
